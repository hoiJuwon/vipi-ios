...
- **Working state:** progress reads like the next assistant message, never a button or card. Italic activity copy and duration, compact English timing (`42s`, `2m 13s`), and the animated Ice Blue luminous core form one leading-aligned vertical stack. The full compact core visibly travels, stretches, and pulses at 30 fps while its Metal texture evolves; extra breathing room separates it from the duration. Reduce Motion removes displacement and texture motion but keeps a gentle opacity pulse so the working state never appears frozen.
...
- **Launch identity:** cold launch uses the selected dark Liquid Halo artwork, then types the small lowercase `vipi` wordmark in a monospaced code face before briefly blinking its terminal-style block cursor. The entire interaction stays within the short launch overlay and fades into the app; Reduce Motion shows the completed wordmark and steady cursor immediately.
...
- **Motion:** remove Liquid Orb displacement and texture evolution under Reduce Motion while retaining a restrained opacity pulse as a live status cue.
