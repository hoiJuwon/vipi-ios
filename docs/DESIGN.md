...
- **Submitted turn:** after a prompt is accepted, its user bubble settles around the upper third of the response viewport with progress directly beneath it. A partial response reserve keeps the working state visible without the large downward jump or near-empty viewport caused by top-pinning a full-screen spacer.
...
- **Session list:** the transcript index follows the compact Messages hierarchy without avatars: centered title, provider selector at the leading edge, Settings at the trailing edge, text-first rows with time and chevron, and a small unread dot. The provider selector contains no status text: it shows the active Vipi or Codex mark with a green connected dot, a compact connecting spinner, or a neutral offline dot, and opens a native selection menu. Search and new-session actions share a floating bottom Glass dock; Settings is navigation, not a persistent footer tab.
