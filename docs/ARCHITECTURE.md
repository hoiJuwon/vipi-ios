...
The mobile session list receives Pi and Codex records in one snapshot but filters them by the explicitly selected provider. Opening history does not resume a Codex thread; both providers return a bounded newest page and load older pages only when the user scrolls or navigates backward. Codex uses cached `thread/turns/list` cursors. Pi reads bounded reverse byte windows from the active read-only JSONL and returns opaque offset cursors, so a 700 MB session does not enter the request path as a full-file parse. Session-list previews similarly read at most a 512 KB tail on cold start and use live runtime metadata afterward. `thread/resume` is deferred until the user sends input. Before resuming a legacy Codex rollout, the adapter invokes Codex's official `migrate-rollouts --apply --thread` command to promote that one thread to paginated history; it never accepts a giant legacy resume response or edits rollout files itself. This keeps existing Pi routing untouched and avoids the Codex desktop app's eager-history cost. Codex approvals are converted to the existing ephemeral interaction card and responses return only to the originating Codex JSON-RPC request.
...
## Cache and latency model

The iOS app persists only normalized session DTOs and the newest 50 user/final-assistant messages per visible session in its protected Application Support container (`0600`, complete-until-first-authentication). On cold launch it renders this snapshot before the network handshake, then revalidates with session snapshots and entry cursors. Raw tool data, Codex rollout content, credentials, and image bytes are not cached in this file. Cache writes are atomic and debounced.

The host never computes a session list by parsing complete JSONL histories. Registry reads are bounded, Pi first-page history is bounded to an 8 MB reverse window, and Codex pages use a small in-memory LRU invalidated by turn/item notifications. These constraints keep list and chat latency independent of total archive size while retaining the single-writer rule.
